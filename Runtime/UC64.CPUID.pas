...
type
  ...
  {Note: when changing TUltraCode64Target, also change UltraCode64TargetStr array
         below}
...
const
  ...
  UltraCode64TargetStr:
  array[Low(TUltraCode64Target)..High(TUltraCode64Target)] of ShortString =
    ('RTLReplacement', 'Blended_IA32', 'Blended_MMX', 'Blended_SSE',
     'Blended_SSE2', 'Dothan', 'Yonah', 'Northwood', 'Presler', 'AMD_64', 'AMD_64X2');

var
  CPU: TCPU;
  UltraCode64Target: TUltraCode64Target;
...
procedure GetUltraCode64Target;
{precondition: GetCPUInfo must have been called}
begin
  UltraCode64Target := fctRTLReplacement;

  if (isSSE2 in CPU.InstructionSupport) then UltraCode64Target:= fctBlendedSSE2
  else if (isSSE in CPU.InstructionSupport) then UltraCode64Target:= fctBlendedSSE
  else if (isSSE in CPU.InstructionSupport) then UltraCode64Target:= fctBlendedSSE
  else if ([isFPU, isMMX, isCMOV] <= CPU.InstructionSupport) then UltraCode64Target:= fctBlendedMMX
  else if ([isFPU, isCMOV] <= CPU.InstructionSupport) then UltraCode64Target:= fctBlendedIA32;

  case CPU.Vendor of
    cvIntel: case CPU.EffFamily of
      6: {Intel P6, P2, P3, PM} case CPU.EffModel of
        PMDothanEffModel: UltraCode64Target:= fctPMD; // Dothan
        PMYonahEffModel: UltraCode64Target:= fctPMY; // Yonah
      end;
      $F: {Intel P4} case CPU.EffModel of
        0, 1, 2: UltraCode64Target:= fctP4N; // Northwood
        6: UltraCode64Target:= fctP4R; // Presler
      end;
    end;
    cvAMD: case CPU.EffFamily of
      $F: {AMD K8} if ((CPU.EffModelBasic = $B) or (CPU.EffModelBasic = $3)) and (isSSE3 in CPU.InstructionSupport) then
          UltraCode64Target:= fctAmd64_SSE3 //AMD X2 dual core CPU
      else UltraCode64Target:= fctAmd64;
    end;
  end;
end;
